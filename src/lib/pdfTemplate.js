...
//
// Style:
//   - Cover (page 1): green top band with company name, large title, address block, green footer band
//   - Content pages (2+): "City Construction Group" + address top-left, logo top-right, divider, footer
//
// All helpers operate on a pdf-lib `PDFDocument`. The caller embeds fonts
// once and passes them in to keep work DRY.
...
export function drawCover(page, fonts, logo, opts) {
  ...
  page.drawRectangle({ x: 0, y: height - bandH, width, height: bandH, color: BRAND.green })

  // Logo (white square with green diamond) on the green band, top-left
  if (logo?.img) {
    const sz = 28
    const gap = 10
    page.drawRectangle({ x: 32, y: height - 56, width: sz, height: sz, color: BRAND.white })
    ...
    page.drawText('cltd.co.uk · info@cltd.co.uk · 0203 948 1930', {
      x: 32, y: height - 60,
      size: 8, font: regFont, color: BRAND.white, opacity: 0.8,
    })
  }
  ...
  if (eyebrow) {
    page.drawText(eyebrow.toUpperCase(), {
      x: 40, y, size: 9, font: regFont, color: BRAND.muted,
  ...
  page.drawText(title, {
    x: 40, y, size: 28, font: boldFont, color: BRAND.text,
  })
  y -= 32

  if (subtitle) {
    page.drawText(subtitle, {
      x: 40, y, size: 18, font: boldFont, color: BRAND.text,
    })
    y -= 26
  }

  // Divider
  page.drawRectangle({ x: 40, y: y - 6, width: width - 80, height: 0.8, color: BRAND.divider })
  y -= 24

  // Project name (bold)
  if (projectName) {
    page.drawText(projectName, { x: 40, y, size: 13, font: boldFont, color: BRAND.text })
    y -= 16
  }
  ...
  for (const line of addressLines) {
    if (!line) continue
    page.drawText(line, { x: 40, y, size: 11, font: regFont, color: BRAND.muted })
    y -= 14
  }
  ...
  page.drawText('PREPARED BY', { x: 40, y, size: 8, font: regFont, color: BRAND.hint })
  y -= 12
  page.drawText(preparedBy, { x: 40, y, size: 11, font: regFont, color: BRAND.text })
  y -= 24

  page.drawText('DATE', { x: 40, y, size: 8, font: regFont, color: BRAND.hint })
  y -= 12
  page.drawText(date, { x: 40, y, size: 11, font: regFont, color: BRAND.text })

  // ─ Bottom green band ─
  const footH = 26
  page.drawRectangle({ x: 0, y: 0, width, height: footH, color: BRAND.green })
  page.drawText('CONFIDENTIAL — CITY CONSTRUCTION LTD', {
    x: 32, y: 9, size: 8, font: boldFont, color: BRAND.white, opacity: 0.95,
  })
}
...
export function drawLetterhead(page, fonts, logo) {
  ...
  page.drawText('City Construction Group', {
    x: 32, y: height - 38, size: 13, font: boldFont, color: BRAND.text,
  })
  page.drawText('One Canada Square, Canary Wharf, London E14 5AA', {
    x: 32, y: height - 52, size: 7.5, font: regFont, color: BRAND.muted,
  })
  page.drawText('T: 0203 948 1930   E: info@cltd.co.uk   W: www.cltd.co.uk', {
    x: 32, y: height - 62, size: 7.5, font: regFont, color: BRAND.muted,
  })
...
export function drawFooter(page, fonts, leftText, pageNum, totalPages) {
  ...
  const { regFont } = fonts
  if (leftText) page.drawText(leftText, { x: 32, y: 18, size: 8, font: regFont, color: BRAND.hint })
  page.drawText(`Page ${pageNum} of ${totalPages}`, {
    x: width - 100, y: 18, size: 8, font: regFont, color: BRAND.hint,
  })
}
