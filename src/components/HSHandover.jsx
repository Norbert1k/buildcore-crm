...
const HS_STRUCTURE = [
  {
    key: 's1', label: 'Pre-Construction', icon: 's1', color: '#448a40',
    children: [
      { key: 's1-f1', label: 'F10 Notification', icon: null, children: [] },
      { key: 's1-f2', label: 'Pre-Construction Information', icon: null, children: [] },
      { key: 's1-f3', label: 'Construction Phase Plan', icon: null, children: [] },
      { key: 's1-f4', label: 'Designer Risk Assessments', icon: null, children: [] },
    ]
  },
  {
    key: 's2', label: 'Site Management', icon: 's2', color: '#378ADD',
    children: [
      { key: 's2-f1', label: 'Site Induction Records', icon: null, children: [] },
      { key: 's2-f2', label: 'Visitor Log', icon: null, children: [] },
      { key: 's2-f3', label: 'Site Rules', icon: null, children: [] },
      { key: 's2-f4', label: 'Emergency Procedures', icon: null, children: [] },
    ]
  },
  {
    key: 's3', label: 'Risk Assessments & Method Statements', icon: 's3', color: '#BA7517',
    children: [
      { key: 's3-f1', label: 'RAMS — General', icon: null, children: [] },
      { key: 's3-f2', label: 'RAMS — Subcontractors', icon: null, children: [] },
      { key: 's3-f3', label: 'Hot Works', icon: null, children: [] },
      { key: 's3-f4', label: 'Confined Spaces', icon: null, children: [] },
    ]
  },
  {
    key: 's4', label: 'Inspections & Monitoring', icon: 's4', color: '#534AB7',
    children: [
      { key: 's4-f1', label: 'Site Inspection Reports', icon: null, children: [] },
      { key: 's4-f2', label: 'Scaffold Inspection', icon: null, children: [] },
      { key: 's4-f3', label: 'Plant & Equipment Checks', icon: null, children: [] },
      { key: 's4-f4', label: 'Environmental Monitoring', icon: null, children: [] },
    ]
  },
  {
    key: 's5', label: 'Incidents & Near Misses', icon: 's5', color: '#E53935',
    children: [
      { key: 's5-f1', label: 'Accident Reports', icon: null, children: [] },
      { key: 's5-f2', label: 'Near Miss Reports', icon: null, children: [] },
      { key: 's5-f3', label: 'RIDDOR Reports', icon: null, children: [] },
      { key: 's5-f4', label: 'Investigations', icon: null, children: [] },
    ]
  },
  {
    key: 's6', label: 'Training & Competency', icon: 's6', color: '#0F6E56',
    children: [
      { key: 's6-f1', label: 'CSCS Cards', icon: null, children: [] },
      { key: 's6-f2', label: 'Training Certificates', icon: null, children: [] },
      { key: 's6-f3', label: 'Toolbox Talk Records', icon: null, children: [] },
      { key: 's6-f4', label: 'Operative Competency', icon: null, children: [] },
    ]
  },
  {
    key: 's7', label: 'Welfare & Environment', icon: 's7', color: '#993C1D',
    children: [
      { key: 's7-f1', label: 'Welfare Facilities', icon: null, children: [] },
      { key: 's7-f2', label: 'Waste Management', icon: null, children: [] },
      { key: 's7-f3', label: 'Noise & Dust Monitoring', icon: null, children: [] },
    ]
  },
  {
    key: 's8', label: 'O&M Manuals & Handover', icon: 's8', color: '#448a40',
    children: [
      { key: 's8-f1', label: 'O&M Manuals', icon: null, children: [] },
      { key: 's8-f2', label: 'As-Built Drawings', icon: null, children: [] },
      { key: 's8-f3', label: 'Warranties & Guarantees', icon: null, children: [] },
      { key: 's8-f4', label: 'Handover Certificate', icon: null, children: [] },
    ]
  },
]

function getAllKeys(nodes, acc = []) {
  for (const n of nodes) { acc.push(n.key); if (n.children?.length) getAllKeys(n.children, acc) }
  return acc
}
...
function findSection(nodes, key) {
  for (const n of nodes) {
    ...
    if (n.children?.length) { const found = findSection(n.children, key); if (found) return found }
  }
  return null
}
...
function fileTypeInfo(name) {
  const n = name || ''
  return {
    isImage: /\.(jpg|jpeg|png|gif|webp)$/i.test(n),
    isPdf: /\.pdf$/i.test(n),
    isWord: /\.docx?$/i.test(n),
    isExcel: /\.xlsx?$/i.test(n),
    isPpt: /\.pptx?$/i.test(n),
  }
}
function FileTypeBadge({ name, size = 34 }) {
  const { isWord, isExcel, isPpt } = fileTypeInfo(name)
  ...
  return (
    <div style={{ width: size, height: size + 8, background: color, borderRadius: 5, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={onCancel}>
      <div style={{ background: 'var(--surface)', borderRadius: 10, padding: 24, maxWidth: 360, width: '90%' }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          ...
          <button onClick={onOk} style={BRs}>Delete</button>
        </div>
      </div>
    </div>
  )
}
const Bs  = { fontSize: 11, lineHeight: '24px', padding: '0 9px', margin: 0, border: '0.5px solid var(--border)', borderRadius: 5, background: 'transparent', cursor: 'pointer', color: 'var(--text2)', display: 'inline-block', alignSelf: 'center', whiteSpace: 'nowrap', flexShrink: 0 }
const BGs = { fontSize: 11, lineHeight: '24px', padding: '0 9px', margin: 0, border: '0.5px solid #448a40', borderRadius: 5, background: 'transparent', cursor: 'pointer', color: '#448a40', display: 'inline-block', alignSelf: 'center', whiteSpace: 'nowrap', flexShrink: 0 }
const BRs = { fontSize: 11, lineHeight: '24px', padding: '0 9px', margin: 0, border: '0.5px solid var(--red-border)', borderRadius: 5, background: 'transparent', cursor: 'pointer', color: 'var(--red)', display: 'inline-block', alignSelf: 'center', whiteSpace: 'nowrap', flexShrink: 0 }
...
function HSFileCard({ file, onPreview, onDelete, canDelete, selected, onSelect }) {
  ...
  return (
    <>
      <div draggable={!renaming} style={{ border: selected ? '2px solid var(--accent)' : '0.5px solid var(--border)', borderRadius: 8, overflow: 'hidden', background: 'var(--surface)', position: 'relative', transition: 'border .1s' }}>
        ...
        <div style={{ height: 120, background: 'var(--surface2)', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', cursor: 'pointer' }} onClick={() => onPreview(file, url)}>
          {isImage && url
            ...
            : <FileTypeBadge name={file.file_name} size={34} />
          }
      ...
      {confirmDel && <ConfirmDialog message={'Delete "' + file.file_name + '"?'} onOk={() => { setConfirmDel(false); onDelete(file) }} onCancel={() => setConfirmDel(false)} />}
    </>
  )
}
...
function HSFileListRow({ file, onPreview, onDelete, canDelete, selected, onSelect }) {
  ...
  const [renaming, setRenaming] = useState(false)
  const [renameVal, setRenameVal] = useState('')
  const { isWord, isExcel, isPpt, isPdf, isImage } = fileTypeInfo(file.file_name)
...
function FolderNode({ node, projectId, depth, fileCounts, canManage, canAddFolders, customFolders, onCustomFolderAdded, sectionColor, onPreview, viewMode, setViewMode }) {
  ...
  async function upload(fileList) {
    ...
    for (const file of fileList) {
      ...
      if (!error) await supabase.from('hs_files').insert({ project_id: projectId, folder_key: node.key, storage_path: path, file_name: file.name, file_size: file.size })
    }
  ...
  return (
    <div style={{ marginBottom: depth === 0 ? 6 : 2 }}>
      <div onClick={() => setOpen(o => !o)} onDragOver={e => e.preventDefault()} onDrop={onDrop}
        style={{ display: 'flex', alignItems: 'center', gap: 10, padding: depth === 0 ? '10px 14px' : '7px 10px', paddingLeft: (depth === 0 ? 14 : 10) + indent, borderRadius: 8, cursor: 'pointer', background: open ? 'var(--surface2)' : 'var(--surface)', border: `0.5px solid var(--border)`, borderLeftWidth: 3, borderLeftColor: color, transition: 'background 0.1s' }}
        ...
        {Icon && (
          <div style={{ width: depth === 0 ? 34 : 26, height: depth === 0 ? 34 : 26, borderRadius: 6, background: color + '20', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <Icon color={color} size={depth === 0 ? 18 : 14} />
          </div>
        )}
        {!Icon && depth > 0 && <div style={{ width: 8, height: 8, borderRadius: '50%', background: color, flexShrink: 0 }} />}
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: depth === 0 ? 13 : 12, fontWeight: 600, color: 'var(--text)' }}>{node.label}</div>
          {depth === 0 && <div style={{ fontSize: 10, color: 'var(--text3)', marginTop: 1 }}>{fileCount > 0 ? fileCount + ' file' + (fileCount !== 1 ? 's' : '') : 'Empty'}</div>}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }} onClick={e => e.stopPropagation()}>
          {showAddFolder ? (
            <>
              ...
              <button onClick={addCustomFolder} style={BGs}>Add</button>
              <button onClick={() => setShowAddFolder(false)} style={Bs}>✕</button>
            </>
          ) : (
            <>
              {open && (
                <button onClick={zipAll} style={{ ...Bs, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
              ...
              {canAddFolders && <button onClick={() => setShowAddFolder(true)} style={Bs}>+ Sub</button>}
              {canManage && (
                <label style={BGs}>
                  {uploading ? '...' : '+ Upload'}
              ...
              {open && depth === 0 && <ViewToggle viewMode={viewMode} setView={setViewMode} />}
            </>
          )}
        </div>
      ...
      {open && (
        <div onDragOver={e => e.preventDefault()} onDrop={onDrop}
          ...
          {childCustomFolders.map(cf => (
            <FolderNode key={cf.folder_key} node={{ key: cf.folder_key, label: cf.label, icon: null, children: [] }}
              projectId={projectId} depth={depth + 1} fileCounts={fileCounts} canManage={canManage}
              canAddFolders={canAddFolders} customFolders={customFolders} onCustomFolderAdded={onCustomFolderAdded}
              sectionColor={color} onPreview={onPreview} viewMode={viewMode} setViewMode={setViewMode} />
          ))}
...
export default function HSHandover({ projectId, projectName }) {
  ...
  async function compileHandover(sectionKeys, filename) {
    ...
    cover.drawRectangle({ x: 0, y: 0, width: 595, height: 842, color: rgb(0.97, 0.97, 0.97) })
    cover.drawRectangle({ x: 0, y: 700, width: 595, height: 142, color: rgb(0.27, 0.54, 0.25) })
    cover.drawText('H&S Handover', { x: 50, y: 780, size: 32, font: boldFont, color: rgb(1,1,1) })
    cover.drawText(projectName || 'Project', { x: 50, y: 740, size: 18, font: regFont, color: rgb(1,1,1) })
    cover.drawText('Generated: ' + new Date().toLocaleDateString('en-GB'), { x: 50, y: 650, size: 12, font: regFont, color: rgb(0.4,0.4,0.4) })
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        <button
          onClick={async () => { setCompilingFull(true); await compileHandover(getAllKeys(HS_STRUCTURE), (projectName || 'project') + '-hs-handover-full.pdf'); setCompilingFull(false) }}
          disabled={compilingFull}
          style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, padding: '7px 14px', border: '0.5px solid #448a40', borderRadius: 6, background: 'transparent', color: '#448a40', cursor: 'pointer' }}>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><polyline points="9 12 11 14 15 10"/></svg>
          {compilingFull ? 'Compiling...' : 'Compile Full H&S Handover'}
        </button>
        <button
          onClick={async () => { setCompilingOm(true); await compileHandover([HS_STRUCTURE[HS_STRUCTURE.length - 1].key], (projectName || 'project') + '-om-manual.pdf'); setCompilingOm(false) }}
          disabled={compilingOm}
          style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, padding: '7px 14px', border: '0.5px solid var(--border)', borderRadius: 6, background: 'transparent', color: 'var(--text2)', cursor: 'pointer' }}>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
          {compilingOm ? 'Compiling...' : 'Compile O&M Manual'}
        </button>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {HS_STRUCTURE.map(node => (
          <FolderNode key={node.key} node={node} projectId={projectId} depth={0}
            fileCounts={fileCounts} canManage={canManage} canAddFolders={canAddFolders}
            customFolders={customFolders} onCustomFolderAdded={cf => setCustomFolders(prev => [...prev, cf])}
