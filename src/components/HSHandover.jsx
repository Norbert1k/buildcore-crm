    ...
    <>
      <div draggable={!renaming} style={{ border: selected ? '2px solid var(--accent)' : '0.5px solid var(--border)', borderRadius: 8, overflow: 'hidden', background: 'var(--surface)', position: 'relative', transition: 'border .1s' }}>
        ...
        <div style={{ padding: '6px 8px' }}>
          ...
          <div style={{ fontSize: 10, color: 'var(--text3)', marginBottom: 5 }}>{fmtSz(file.file_size)}</div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {renaming
            ...
            : <div onClick={() => onPreview(file, url)} style={{ cursor: 'pointer' }}>
                ...
                <div style={{ fontSize: 10, color: 'var(--text3)', marginTop: 2 }}>{fmtSz(file.file_size)}</div>
              </div>
          }
        </div>
...
function HSFilesGrid({ files, viewMode, onPreview, canManage, onDelete, selected, onSelect, onDrop, onUpload }) {
  ...
  if (viewMode === 'list') return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      {files.map(f => <HSFileListRow key={f.id} file={f} onPreview={onPreview} canDelete={canManage} onDelete={onDelete} selected={selected.has(f.id)} onSelect={onSelect} />)}
    </div>
  )
  return (
    <div style={{ display: 'grid', gridTemplateColumns: viewMode === 'compact' ? 'repeat(auto-fill, minmax(110px, 1fr))' : 'repeat(auto-fill, minmax(150px, 1fr))', gap: viewMode === 'compact' ? 6 : 8 }}>
      {files.map(f => <HSFileCard key={f.id} file={f} onPreview={onPreview} canDelete={canManage} onDelete={onDelete} selected={selected.has(f.id)} onSelect={onSelect} />)}
      {canManage && (
...
// ── Folder Node ───────────────────────────────────────────────────────────────
function FolderNode({ node, projectId, depth, fileCounts, canManage, canAddFolders, customFolders, onCustomFolderAdded, sectionColor, onPreview, viewMode }) {
  ...
  const Icon = HS_ICONS[node.icon] || null
  ...
  const hasChildren = (node.children?.length || 0) + childCustomFolders.length > 0
  const indent = depth * 16

  return (
    <div style={{ marginBottom: depth === 0 ? 6 : 2 }}>
      <div onClick={() => setOpen(o => !o)} onDragOver={e => e.preventDefault()} onDrop={onDrop}
        style={{ display: 'flex', alignItems: 'center', gap: 10, padding: depth === 0 ? '10px 14px' : '7px 10px', paddingLeft: (depth === 0 ? 14 : 10) + indent, borderRadius: 8, cursor: 'pointer', background: open ? 'var(--surface2)' : 'var(--surface)', border: '0.5px solid var(--border)', borderLeft: '3px solid ' + color, transition: 'background .1s' }}
        ...
        {Icon && <div style={{ width: depth === 0 ? 34 : 26, height: depth === 0 ? 34 : 26, borderRadius: 6, background: color + '20', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Icon color={color} size={depth === 0 ? 18 : 14} /></div>}
        <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }} onClick={e => e.stopPropagation()}>
          {showAddFolder ? (
            ...
            <>
              {open && <button onClick={zipAll} style={{ ...Bs, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
              ...
              {open && <ViewToggle viewMode={viewMode} setView={v => { try { localStorage.setItem('hsView_' + projectId, v) } catch {} window.dispatchEvent(new CustomEvent('hsViewChange', { detail: v })) }} />}
            </>
          )}
        </div>
        ...
        <div onDragOver={e => e.preventDefault()} onDrop={onDrop}
          ...
          {node.children?.map(child => (
            <FolderNode key={child.key} node={child} projectId={projectId} depth={depth + 1}
              ...
              sectionColor={color} onPreview={onPreview} viewMode={viewMode} />
          ))}
          {childCustomFolders.map(cf => (
            <FolderNode key={cf.folder_key} node={{ key: cf.folder_key, label: cf.label, children: [] }} projectId={projectId} depth={depth + 1}
              fileCounts={fileCounts} canManage={canManage} canAddFolders={canAddFolders}
              customFolders={customFolders} onCustomFolderAdded={onCustomFolderAdded}
              sectionColor={color} onPreview={onPreview} viewMode={viewMode} />
          ))}
