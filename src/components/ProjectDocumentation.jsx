...
function FolderIcon({ color, bg, size = 20 }) {
  return (
...
function FileCard({ file, onDownload, onDelete, canDelete }) {
  ...
  return (
    <div style={{ border: '0.5px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden', background: 'var(--surface)' }}>
      <div style={{ height: 130, background: 'var(--surface2)', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
        {isImage && url ? (
          ...
          <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="var(--text3)" strokeWidth="1">
      ...
      <div style={{ padding: '8px 10px' }}>
        <div style={{ fontSize: 11, fontWeight: 500, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginBottom: 2 }} title={file.file_name}>
          {file.file_name}
        </div>
        <div style={{ fontSize: 10, color: 'var(--text3)', marginBottom: 7 }}>
          {fmtSize(file.file_size)}{file.file_size ? ' · ' : ''}{new Date(file.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
        </div>
        <div style={{ display: 'flex', gap: 4 }}>
          {url && (
            <button onClick={() => window.open(url, '_blank')} style={{ flex: 1, fontSize: 10, padding: '4px 0', border: '0.5px solid var(--border)', borderRadius: 4, background: 'transparent', cursor: 'pointer', color: 'var(--text2)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3 }}>
              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
              View
            </button>
          )}
          <button onClick={onDownload} style={{ flex: 1, fontSize: 10, padding: '4px 0', border: '0.5px solid var(--border)', borderRadius: 4, background: 'transparent', cursor: 'pointer', color: 'var(--text2)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3 }}>
          ...
          {canDelete && (
            <button onClick={onDelete} style={{ fontSize: 10, padding: '4px 7px', border: '0.5px solid var(--red-border)', borderRadius: 4, background: 'transparent', cursor: 'pointer', color: 'var(--red)' }}>✕</button>
          )}
        </div>
      </div>
    </div>
...
function SubfolderSection({ projectId, folder, subfolder, canManage }) {
  ...
  return (
    <div style={{ marginBottom: 3 }}>
      ...
      >
        <FolderIcon color={folder.color} bg={folder.bg} size={14} />
...
function PrimeFolderSection({ projectId, folder, canManage, canAddFolders, allFileCounts }) {
  ...
  return (
    <div style={{ marginBottom: 6 }}>
      ...
      >
        <FolderIcon color={folder.color} bg={folder.bg} />
        <div style={{ flex: 1 }}>
